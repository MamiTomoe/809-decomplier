...
void Logic::parseAssemblyInfo()
{
	...
	{
		if (START_FORM_FILE == _assemblyFromFile[i] || END_FROM_FILE == _assemblyFromFile[i])
		{
			//TODO: Put here stack frame
		}
		else if (auto found = _foundSymbols.find(_assemblyFromFile[i]); found != _foundSymbols.end())
		{
			if (isOpcodeVarsFromTheFileAreLegal(found->second.second + i)){
				_insturctions.push_back(_assemblyFromFile[i]);
				_destVars.push_back(_assemblyFromFile[i + DEST_VASR_INDEX]);
				_sourceVars.push_back(_assemblyFromFile[i + SRC_VARS_INDEX]);
				i += found->second.second;
			}
			else {
		...
		else if (isInstructuionNotLast(i))
		{
			if (auto found = _foundSymbols.find(_assemblyFromFile[i + 1]); found == _foundSymbols.end()) {
				...
				if (PUT_STACK_VARS_IN_ASSEMBLY == _assemblyFromFile[i]) {
					_stackAndHeapVars.push_back(_assemblyFromFile[i + FUNCTION_OR_STACK_VARS_INDEX]);
				}
				else if (FUNCTION_INSTRCATION == _assemblyFromFile[i]) {
					_stackAndHeapVars.push_back(FUNCTION_INSTRCATION);
					_functionNames.push_back(_assemblyFromFile[i + FUNCTION_OR_STACK_VARS_INDEX]);
				}
				else if (REMOVE_STACK_VARS_IN_ASSEMBLY == _assemblyFromFile[i]) {
					_stackAndHeapVars.push_back(_assemblyFromFile[i + FUNCTION_OR_STACK_VARS_INDEX]);
				}
				i += NEXT_INDEX_REGULAR;
			}
			else{
				//TODO: 
			}
 ...
 **/
 ...
 Logic::Logic()
 {
	 try
	 {
		 ...
		 checkAndGetSymbols();
		 parseAssemblyInfo();
		 for (const auto& i : _stackAndHeapVars) { std::cout << i << '\n'; }
		 for (const auto& i : _functionNames) { std::cout << i << '\n'; }
	 }
